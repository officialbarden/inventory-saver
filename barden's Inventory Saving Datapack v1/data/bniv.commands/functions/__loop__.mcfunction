...
execute as @a[scores={bniv-nameTag-dropped=1..},predicate=bniv.commands:is_sneaking] at @s if entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:name_tag",tag:{bniv_nametag:1b}}},sort=nearest,limit=1] run function bniv.commands:commands/save/extras/change_name/__2__
